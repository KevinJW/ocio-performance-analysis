...
def analyze_ocio_241_vs_242():
    ...
    # Load the data
    csv_file = Path("ocio_test_results.csv")
    ...
    if comparisons:
        ...
        if abs(avg_improvement) < 2:
            print(f"   ✅ Performance is very similar between versions")
        elif avg_improvement < -5:
            print(f"   🚀 OCIO 2.4.2 shows meaningful improvement")
        elif avg_improvement > 5:
            print(f"   ⚠️  OCIO 2.4.1 performs better on average")
        else:
            print(f"   📊 Minor performance differences between versions")
